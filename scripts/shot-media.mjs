...
for (let i = 0; i < 5; i++) {
  ...
  await new Promise((r) => setTimeout(r, 2200)); // swiper init + videos paint
  await page.mouse.move(700, 40);
  await page.screenshot({ path: `_content/_shots/mslider-${i + 1}.jpg`, type: "jpeg", quality: 84 });
  console.log("mslider-" + (i + 1));
}
await browser.close();
console.log("done");
